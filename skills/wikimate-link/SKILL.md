...
6. **결과 보고 + 검증**: 생성/갱신된 노트 경로(`note`)와 최종 멤버(`resulting_members`)를 확인한 뒤에만 "완료"라 말한다. **기존 MOC가 있었다면 사용자가 직접 쓴 다른 섹션("## 왜 MOC가 필요한가" 등)이 그대로 보존됐는지도 같이 확인**한다(surgical 치환이라 보존되는 게 정상이지만, 과신 대신 확인). 그래프뷰 확인은 워크플로우 A와 동일하게 사용자에게 안내.
