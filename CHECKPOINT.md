...
## 다음 단계 (2026-09-11 확정 — 최신, 아래가 최신 우선순위. 위 2026-09-01 절은 참고용 이력)

### 1순위 · `v0.10.0` GitHub Release 발행 (태그는 이미 있음, 노트만 필요 — 사람 전담)
**왜 1순위인가**: 코드·문서 변경이 전혀 필요 없고(태그·`package.json` 이미 `0.10.0`), 사용자의 기존 범용 규칙과 직접 어긋난 상태로 방치 중인 유일한 항목. 위험 대비 효과가 가장 명확함.
**AI가 준비한 것**: 0.9.0 이후 변경분 요약(status/project 자연어 트리거, `Link.reason`/`Link.kind` 구현, 노션 Run Log 5개 스킬 확장, MOC 중복·`replace_link` 존재검증 등 실결함 수정, 노션·마켓플레이스 라이브 검증 완료) — 실행은 사용자가 `gh release create v0.10.0` 직접 실행(정책상 AI 실행 금지).
**위험**: 낮음(공개 저장소에 보이는 행동이라 최종 문구는 사용자 확인 필요).

### 2순위 · 이 `CHECKPOINT.md` 커밋
**현재 상태**: 이번 갱신으로 정정 완료 — 커밋만 하면 됨. 위험 없음(문서 파일, 코드 영향 0).
...
- `.active-agents*`(3개)·`.sodam-re/safety-log.jsonl` 미추적 파일: `.gitignore` 미등록 상태로 여러 세션째 방치 확인(2026-09-11). `.gitignore` 등록 여부·삭제 여부 결정 필요하나 이번 지시 범위(CHECKPOINT/PRD 정합성) 밖이라 판단만 남기고 손대지 않음.
