...
## 🟡 2026-08-21 갱신(5) — 다음 단계 확정 + 리스크 전수 정리 (실행은 다음 세션)

> 위 갱신(1)~(4)로 이 세션에서 실행 가능했던 항목은 소진. "다음은 뭘 해야 하나"를 다시 물어와 코드를 직접 재grep해 **이전에 한 번도 보고된 적 없던 새 배선 결함 1건**을 추가로 확인. 아래는 실행 우선순위와 각 항목의 위험을 정리만 한 것 — 이번 갱신에서 코드/스킬 자체는 아직 안 고침.

### 1순위 — `wikimate-classify` 스킬의 "완료 표시" 자연어 트리거 배선 누락 (즉시 착수 가능, 위험 낮음)
- **현상(직접 확인)**: `status`(진행상태: inbox/draft/done)·`project`(관련 프로젝트) 기능은 `mcp/lib/classify.mjs`·`mcp/server.mjs` 스키마·`README.md`/`README.en.md`(310행, "이 노트 다 봤어, 완료로 표시해줘" 예문까지 있음) 전부 완성돼 있는데, `skills/wikimate-classify/SKILL.md`의 YAML `description`(자동 트리거 판단 기준)과 워크플로우 본문 둘 다 status/project를 **0건** 언급함(grep 재확인). README가 안내하는 발화를 실제로 하면, 정작 그 발화로 켜져야 할 스킬 설명서엔 그 상황이 없어 에이전트가 이 기능의 존재를 모를 수 있음.
- **동일 계열 선례**: 2026-08-18 "MOC(목차) 만들어줘가 안 먹히던" 결함과 근본 원인이 같음(로직은 있는데 자연어→스킬 연결 배선만 빠짐) — 그때와 같은 방식(트리거 문구·워크플로우 단계 보강)으로 고치면 됨.
- **안 고치면 생기는 위험**: 사용자 체감상 "분명 되는 기능이라던데 안 먹힌다"는 혼란 — 기능 자체는 멀쩡한데 배선 문제라 디버깅 방향을 잘못 잡기 쉬움(코드를 의심하게 됨).
- **고칠 때 예상 위험(고려 후 대응 방향)**:
  - `description`에 문구를 추가할 때 "프로젝트"·"완료" 같은 흔한 단어를 넣으면 `wikimate-organize`(정리) 등 **다른 스킬과 트리거가 겹칠 가능성** — organize 스킬의 트리거 문구와 겹치지 않는지 대조하며 추가할 것.
  - 워크플로우 본문에 status/project 판단 단계를 넣을 때 **기존 folder/tags 판단 순서·문구를 건드리지 않고 추가만** 할 것(과거 세션에서 이미 검증된 흐름이라 손대면 회귀 위험).
  - 이 수정도 **실제 자연어 자동발동 여부는 프로그램으로 검증 불가** — 세션 재시작 후 사람이 직접 "완료로 표시해줘" 같은 말로 확인해야 함(2026-08-18 MOC 수정 때와 동일한 한계, 과신 금지).
  - 지난 라운드에서 이미 한 번 이 필드를 추가했다가 "범위 밖"이라 되돌린 이력이 있음(2026-08-21 갱신(2)) — 이번엔 그 자체가 목적이므로 되돌리지 않음.

### 2순위 — `Link.reason`/`Link.kind` 미구현 (설계 결정 선행 필요, 성급한 착수 금지)
- **현상**: `02_DATA_MODEL.md`의 Link 엔티티가 `reason`(왜 연결했는지)·`kind`(관계 종류: related/reference) 둘 다 정의해뒀지만, `mcp/lib/link.mjs`엔 저장 공간 자체가 없음(에러 메시지용 `reason:` 필드만 있고 데이터 필드 아님 — grep 재확인. `kind`는 이번에 처음 grep해서 코드 0건 확인, 이전 세션 기록엔 없던 새 확인).
- **왜 보류인지**: 현재 `related:` frontmatter는 "한 줄 배열"만 읽는 자체 정규식 파서(진짜 YAML 아님)라, 링크 하나하나에 이유·종류를 붙이려면 저장 구조 자체를 바꿔야 함 — 이건 코드 버그가 아니라 **설계 선택**의 문제.
- **성급하게 진행할 때의 위험**: (a) 본문에 "## 왜 연결했는지" 섹션을 병행하는 방식 → 파서 안 바꿔도 되지만 frontmatter와 본문 두 곳에 관련 정보가 흩어짐(일관성 저하). (b) `related:` 자체를 객체 배열로 바꾸는 방식 → 표현력은 좋아지지만 **기존에 이미 저장된 모든 노트의 `related:` 값과 호환이 깨짐**(대규모 마이그레이션 필요, 이 세션의 "범위 밖 대규모 리팩토링 금지" 원칙과 정면 충돌). 둘 다 사용자 승인 없이 AI가 임의로 고를 사안이 아님.
- **결론**: 다음 세션에서 "제안하기" 형식으로 (a)/(b) 중 방향을 먼저 정하고, 그 뒤에 구현.

### Phase 정의 재확인 (새 발견 아님, 근거 재확인)
`03_PHASES.md`를 직접 다시 읽어 Phase -1/1a/1b/2/3까지만 있고 **Phase 4는 존재하지 않음**을 재확인. "다음 Phase"라는 새 단계는 없고, 위 1·2순위는 전부 Phase 3 범위 내 잔여 갭.

### 모순 점검 결과 (요청된 "모순 없이" 기준 점검)
이번 재검토에서 발견한 유일한 모순은 위 1순위 그 자체(README는 기능이 된다고 안내 vs 스킬 배선은 그 상황을 모름). 그 외 README/AGENTS.md/서버 스키마/코드 4곳을 status·project·reason·kind 키워드로 교차 대조했을 때 **추가 모순은 발견 못 함**(못 찾은 것이지 "없다고 확정"은 아님 — 이 네 문서 조합에 한정된 점검이라는 한계는 정직히 남김).
...
## 🟢 2026-08-21 갱신(7) — 세션 종료 · 다음 세션 Resume 지침 (참고용 이력 — 최신은 "2026-08-31 갱신" 절)
...
## 🟢 2026-08-31 갱신 — Link.reason 구현 + classify 트리거 배선 보강 + 노션 안전 규칙 2건 신규 발견·해결 (참고용 이력 — 최신은 "2026-09-01 갱신" 절)

> 10일간 정지 상태였던 프로젝트(마지막 커밋 2026-08-21 `b80836d`)를 재개. 시작 전 `npm run verify` 191/191 재실행 + GitHub Actions 최근 3회 success로 "문서가 주장하는 상태 = 실제 상태" 여부부터 직접 재검증(추측 아님). 이후 `04_PROJECT_SPEC.md` §4 "절대 하지 마" 13개 항목을 코드/스킬과 전수 재대조하다가 지난 10여 회 감사에서 한 번도 안 잡힌 새 결함 2건을 발견.

### 1) 🔴 신규 발견: 노션 안전 규칙 2건이 "절대 하지 마"인데 코드·스킬 어디에도 구현/안내가 0건
- **① 권한 스코프**: "노션 전체 워크스페이스 권한을 연결하지 마 — 지정 DB(Research·Run Log)만"이 README·AGENTS.md·6개 스킬·`.env.example` 전체에 안내 문구 0건(grep 재확인).
- **② rate-limit**: `03_PHASES.md` Phase 1b 명시 항목("rate-limit 재시도·페이지네이션")이 코드·스킬 어디에도 0건.
- **왜 지금까지 안 잡혔나**: 노션 색인은 설계상 "코어 밖(스킬 레벨)"이라 `npm run verify`(코드 자동테스트) 범위에 애초에 안 걸림 — 이번엔 코드가 아니라 스킬 프롬프트 텍스트를 직접 대조해서 발견.
- **조치**(문서/스킬 텍스트만, 코드 0): `AGENTS.md`(접근 규칙·안전 규칙 절)와 `skills/wikimate-organize/SKILL.md`(노션 색인 절 + 병렬 수집 절)에 두 원칙 추가 — 신규 연결 시 두 DB만 공유 안내, 대량 쓰기 시 간격+재시도 1~2회+실패해도 격리(다른 자료로 전파 안 됨)+Run Log에 정직히 기록.

### 2) `wikimate-classify` status/project 자연어 트리거 배선 보강(2026-08-21 갱신(5)에서 이미 확정된 항목 실행)
- `skills/wikimate-classify/SKILL.md`의 YAML `description`·워크플로우·도구 파라미터 목록(`apply`)에 `status`/`project` 관련 문구가 0건이던 것을 보강 — "완료로 표시해줘"/"프로젝트 지정해줘" 등 트리거 문구 추가, 워크플로우 3번에 판단 단계(자동 전환 금지 원칙 명시), 도구 설명에 `status`/`project` 파라미터 정식 문서화(과거 누락). `commands/wikimate-classify.md`도 동일 반영.

### 3) `Link.reason` 구현 — 본문 "## 왜 연결했는지" 섹션 병행(2026-08-31 사용자 확정 방향)
- **설계**: `related:` frontmatter 파서가 "한 줄 배열만" 읽는 자체 정규식이라 구조 변경은 기존 노트 전체 마이그레이션 위험 → **frontmatter는 그대로 두고 본문에 병행**(무위험·되돌리기 쉬움, `.PRD/02_DATA_MODEL.md` "결정됨" 절에 기록). `reason`을 안 넘기면 섹션 자체가 안 생겨 기존 동작에 전혀 영향 없음(완전 하위호환).
- **구현**: `mcp/lib/link.mjs` — `build_moc`이 쓰던 surgical 섹션 치환 로직(`findMocMembersRange` 등)을 `findHeadingSectionRange`로 일반화해 MOC 섹션과 reason 섹션이 공유(중복 로직 방지, 기존 stripQuotes/SECRET_PATTERNS 통합과 동일한 이유). `add_links`에 `reason`(선택) 추가 — 기존 이유 불릿은 보존하고 새 불릿만 append(재작성 아님), 개행은 한 줄로 정규화. `mcp/server.mjs` 스키마·핸들러에 `reason` 배선. `skills/wikimate-link/SKILL.md`·`commands/wikimate-link.md`·`AGENTS.md`에 사용법 반영.
- **검증**: `verify-link.mjs`에 신규 회귀 15개(하위호환·dry-run 미리보기·surgical 치환·개행 정규화·append 보존·사용자가 직접 쓴 다른 섹션 보존) — 59→**74**. `smoke-tools.mjs`에 서버경유 실배선 확인 1개 추가(reason 인자가 JSON-RPC를 거쳐 실제로 본문에 반영되는지) — 14→**16**. `npm run verify` 총계 191→**206**(전부 PASS, 회귀 0). `security-scan.mjs --all` 72개 통과. `node --check` 전체 clean.
...
### 실행 내용
- **README.md/README.en.md**: 상단 배너·§7·§12의 테스트 개수(191→206)·날짜(2026-08-21→2026-08-31) 갱신. §13 업데이트 요약에 새 토글 1개 추가(Link.reason 구현·classify 트리거 실제 보강·노션 안전규칙 2건 문서화) — **단 "main 커밋·푸시는 아직"이라고 정직하게 명시**(작성 시점에 `git status`로 실제 미커밋 재확인). §17 상태표 행 3개 갱신(분류 트리거 ✅·Link.reason ✅ 신규 분리·노션 안전규칙 ✅ 신규 추가), "앞으로 남은 것" 토글에서 이미 끝난 항목(분류 트리거) 제거하고 Link.kind만 남김.
- **독립적으로 새로 발견한 사소 오기 1건**: §14 폴더 구조가 "e2e 6개"로 적혀 있었으나(2026-08-21부터), `ls scripts/`로 직접 재확인한 결과 `e2e-*.mjs` 패턴 파일은 실제로 **5개**(2026-08-21 당시 "4종+1개 추가=6개" 산술 오류로 추정, 4+1=5가 맞음). ko/en 둘 다 5개로 정정.
- **`.PRD/01_PRD.md` §5 성공 기준**: 생성 이후 8개 항목이 전부 `[ ]`로 방치돼 있었으나 실제로는 6개가 이미 코드·테스트로 충족돼 있었음 → 근거 각주와 함께 체크. 나머지 2개(Codex 자연어 실사용 라이브 트리거, `notion_id` 실계정 클릭 왕복)는 **사람만 확인 가능한 라이브 검증이 남아있어 의도적으로 미체크 유지**(과장 금지 원칙 — 코드 완성과 라이브 검증을 구분).
- **README.html/README.en.html 재생성**: 2026-08-21에 확립한 방법(pandoc으로 md에서 새로 생성 + 기존 CSS 헤더 재사용 + TOC를 `<nav class="toc">`로 재포장) 그대로 재사용(제목 텍스트를 하나도 안 바꿔서 앵커 id 전부 그대로 유지됨, 사전 확인 완료). 재생성 직후 h2(22)·h3(29 — 최초 단순 grep은 pandoc이 속성을 줄바꿈해서 27로 오검출됐다가 개행-내성 재검사로 실제 29 일치 확인, 콘텐츠 손실 아니었음)·details(6→7, 신규 토글 1개분) 개수와 "206"·"2026-08-31" 등장 횟수가 ko/en 둘 다 정확히 일치함을 직접 카운트로 확인.

### 검증
`npm run verify` 206/206(exit 0, 회귀 없음, 코드 미변경이라 당연하지만 직접 재확인) · `security-scan.mjs --all` 72개 전수 통과. 코드 변경 0 — 문서 6개 파일만 변경(`.PRD/01_PRD.md`, `README.md`, `README.en.md`, `README.html`, `README.en.html`, 이 `CHECKPOINT.md`).

### [후속 4, 이 계열 마지막] `03_PHASES.md` Phase 3 상태표 정정 — 그리고 문서 사냥 중단 판단
`03_PHASES.md`의 "Phase 로드맵 요약" 표가 여전히 Phase 3를 "시작 전"으로 표기 — 이 표 자신의 각주가 가리키는 `.PRD/README.md`(Phase 3 항목 대부분 2026-08-20 완료로 이미 기록됨)와 모순됨을 발견·정정(🟢 대부분 완료로 갱신, 남은 건 마켓플레이스 신규설치 라이브 검증뿐). `npm run verify` 206/206·`security-scan.mjs --all` 72개 재확인.
**판단**: 같은 "PRD 전수 재독" 요청이 5차례 반복되며 매번 실제 문서 결함을 찾아 고쳤으나(테스트 개수→README 낡음→미결사항 오류→3문서 모순→Phase 표), 이제 문서 정합성 사냥은 소진 단계에 도달했다고 판단. **더 이상 새 문서 결함을 찾으러 다니지 않고**, 대신 실질적 위험(20개 파일이 미커밋 상태로 계속 누적되는 것)을 사용자에게 직접 제시하기로 함 — 본래 구현 목적(정리 도구가 스스로도 잘 정리돼 있을 것)은 이미 충분히 달성됐다고 판단.

### [후속 3] 3개 PRD 문서 간 "미결 사항" 모순 발견·해소(제가 직전에 만든 모순 포함)
직전 절([후속 2])에서 `.PRD/README.md`의 요약본만 고치고 그 원본인 `01_PRD.md §7`·`04_PROJECT_SPEC.md §10`은 안 건드려서, **같은 질문에 세 문서가 서로 다른 답을 하는 상태**가 됐음을 재확인 후 즉시 발견·정정. `01_PRD.md §7`(6개 중 5개 체크, 노션 접근만 유지), `04_PROJECT_SPEC.md §10`(8개 중 5개 체크, 노션 관련 3개만 유지)을 코드 근거와 함께 정정. 이 과정에서 **`.PRD/README.md`에도 빠뜨렸던 항목 1개를 추가 발견**: "백업 방식"이 `git 커밋(추천)` vs `폴더 복사` 중 미결로 남아 있었는데, `mcp/lib/shared.mjs`의 `backupFile()`(파일을 `.wikimate/backups/<타임스탬프>/`로 `copyFile`)을 직접 확인한 결과 **PRD 추천값이 아닌 "폴더 복사" 쪽으로 이미 구현돼 있음**을 확인해 3개 문서 전부에 반영. 코드 변경 0, `npm run verify` 206/206·`security-scan.mjs --all` 72개 통과 재확인.

### [후속 2] `.PRD/README.md` "미결 사항" 5개 중 4개가 이미 결정·구현된 것 발견·정정
"미결 사항(NEEDS CLARIFICATION)" 목록의 "아키텍처/배포" 5개 항목을 코드와 하나씩 대조한 결과, **MCP 코어 언어·옵시디언 접근·배포 저장소/라이선스·1차 검증 에이전트 4개가 이미 오래전에 결정·구현돼 있는데도 `[ ]`로 방치**돼 있었음을 발견. 근거와 함께 체크(MCP 코어 언어는 "Node/TS"가 아니라 실제로는 **순수 JS**라는 뉘앙스 정정 포함 — TS 채택 안 됨을 명시). **노션 접근 1개는 체크하지 않고 유지** — 하드코딩 선택이 아니라 "연결된 도구 자동 감지"가 설계 의도라 원래도 고정할 필요가 없는 항목이라 억지로 닫지 않음(거짓 완료 방지). "데이터/운영" 목록의 `dry-run 기본값 여부`도 같은 패턴(이미 모든 쓰기 도구의 기본 동작인데 미결로 남아 있던 것)이라 함께 정정. 코드 변경 0, `npm run verify` 206/206 그대로.

### [후속 1] `.PRD/README.md`도 같은 갱신 필요 발견·해결
같은 원칙("다음 작업 방향" 강력 추천 → 사용자 승인 → 진행)으로 재확인한 결과, `.PRD/README.md`(PRD 요약 진입점 문서)가 **① 절 제목이 "2026-08-04 갱신"인데 본문은 2026-08-21까지 기록돼 있는 자기모순**, **② 이번 세션(Link.reason·classify 트리거·노션 안전규칙·README 재동기화) 전체가 반영 안 됨** 두 가지 문제를 갖고 있었음을 발견 — 방금 고친 README.md/CHECKPOINT.md와 똑같은 "문서가 코드보다 먼저 낡는" 패턴이 PRD 요약 문서 자체에서 재현된 것. 정정 조치: ① 절 제목 아래에 "최신은 맨 아래 2026-09-01 절 참고" 정정 안내 추가, ② "2026-08-31"·"2026-09-01" 두 개 신규 절 추가(이번 세션 요약 + 정직성 고지 "커밋·푸시는 아직"), ③ "미결 사항" 목록에 `Link.reason`(결정됨, 체크) · `Link.kind`(미결, 유지) 항목 신설. `npm run verify` 206/206 그대로(문서만, 코드 변경 0).

### 현재 상태 (2026-09-11 재확인 — 위 "커밋/푸시는 여전히 안 함"은 낡은 서술, 아래로 정정)
- **위 20개 파일은 이미 전부 커밋·푸시 완료** — PR #20(`ad68cd8`, CHECKPOINT 기록)·PR #21(`05f6efd`+`86b3184`, 릴리즈 문구 정정 로그)·PR #22(`0316dc4`+`7b97706`, 변경이력 최신순 재정렬)·PR #23(`55ac6b0`+`798c25f`, 라이브 검증 상태 동기화+라이선스 안내)로 나뉘어 순차 병합됨(`git log` 직접 확인, `main == origin/main`). **`CHECKPOINT.md` 1개 파일만** 아직 작업 트리에 남아 있었음(`git status` 재확인).
- **다음 세션 후보**: 아래 "다음 단계(2026-09-11 확정 — 최신)" 절 참고. Codex/Gemini 실사용 포팅은 **사용자가 2026-09-01 명시적으로 보류 결정**("구현이 어느정도 완료되고 잘 쓸 수 있게 되면 그때 포팅") — 다음 세션에서 먼저 꺼내지 말고 사용자가 준비됐다고 할 때까지 대기.
...
### [후속 6] README 변경사항 요약 정리(PR #21·#22) + 노션·마켓플레이스 실사용 라이브 검증 완료(2026-09-01)
**문서**: §13 업데이트 내용 요약에 [후속 5] 작업 내역(GUIDE 삭제·릴리즈 문구 정정) 토글 신규 추가(PR #21) 후, 사용자가 "왜 계속 v0.7.1이 맨 위냐"고 지적 — §13이 오래된→최신 순으로 쌓여 있던 걸 실제 결함으로 확인하고 전체를 최신순(내림차순)으로 재정렬(PR #22, ko/en·md/html 4파일).

**실사용 라이브 검증 — PRD/README가 "사람 전담·미검증"으로 표시해 온 항목 2건이 이번에 실제로 검증됨**:
- **마켓플레이스 새 설치**: 사용자 컴퓨터의 플러그인 캐시가 `v0.7.2`에 멈춰 있던 실결함 발견(8월 9일 이후 갱신 안 됨, `marketplace update`로도 안 고쳐짐) — `/plugin marketplace remove` → `add` → `install` 순서로 해결, 캐시에 `0.10.0` 폴더 생성 확인. 이어서 `.mcp.json`이 요구하는 `OBSIDIAN_VAULT_PATH` 환경변수가 실제 윈도우 환경변수로 설정 안 돼 있던 것도 발견(제가 이전에 "`.env` 파일에 설정"이라고 잘못 안내했던 부분 — 전역 설치된 플러그인은 프로젝트 `.env`를 안 읽음, `setx` 필요) → 사용자가 `setx OBSIDIAN_VAULT_PATH`로 해결, 재시작 후 `/mcp`에 8개 도구(`wikimate_summarize` 포함) 전부 정상 노출·Errors 탭에 wikimate 없음 확인.
- **노션 연결 양방향**: 자연어("이 자료 정리해서 노션에도 색인해줘")는 스킬 자동발동이 실제로 안 걸려 옵시디언 노트가 생성 안 됨(이미 알려진 한계 재현, 새 결함 아님) → `/wikimate <URL>` 슬래시 명령으로 재시도해 100% 정상 작동: `wikimate_collect`로 노트 생성(dry-run 계획→승인) → `wikimate-reviewer` 서브에이전트가 실제로 사실관계 오류 2건(자살률 통계 수치 결합, BLACKPINK 빌보드 순위 오귀속) 발견 → 사용자 승인 후 두 곳 모두 원문 노트에서 직접 수정 → 노션 "Wikimate Research Library" DB에 색인 행 생성 → `notion_id`로 옵시디언 노트에 되쓰기까지 완료. 실제 파일(사용자의 실제 옵시디언 볼트 내 `대한민국.md`, 정확한 로컬 경로는 개인정보라 이 문서에는 생략)의 frontmatter를 직접 열어 `notion_id` 채워짐을 재확인함(추측 아님). 노션 "Wikimate Run Log" DB도 이번에 처음 실제 생성되고 기록됨.
- **Codex·Gemini 라이브 포팅**: 사용자가 명시적으로 보류 결정(위 "다음 세션 후보" 참고) — 이번 세션에서 시도 안 함.

**의미**: `.PRD/README.md`·`README.md` §17이 오랫동안 "🟡 코드 완성, 라이브 미검증"으로 표시해 온 노션 색인·마켓플레이스 신규설치 2개 항목이 이번 세션에서 실제로 사람 계정으로 라이브 검증까지 끝났다. 다음 문서 갱신 시 이 두 항목을 "✅ 완료(실사용 검증됨, 2026-09-01)"로 정정할 것(코드 변경은 없었으므로 `npm run verify` 등 자동 테스트는 그대로 215/215).

이 `CHECKPOINT.md` 갱신은 로컬 파일 수정만 — **커밋/푸시는 사용자 지시 시에만**.

## 🟢 2026-09-11 갱신 — CHECKPOINT.md 자기 정합성 점검: 미커밋 초안의 "1순위"가 실제로는 이미 완료된 상태였음을 발견·정정

> 사용자 요청("CHECKPOINT+PRD 전부 정독 후 다음 작업 방향 강력 추천")으로 이 파일 전체(555줄)·`.PRD` 6개 문서·`git diff`·원격 태그·GitHub Release를 전수 재대조하던 중 발견.

- **발견(직접 대조 확인, 추측 아님)**: 바로 아래 "다음 단계(2026-09-01 확정)"의 1순위가 "README.md 615·616·478·481줄의 '라이브 미검증' 표기를 정정해야 함"이라고 **미완료 과제**로 적어뒀는데, `git log` 최상단 4개 커밋(PR #21 `05f6efd`/`86b3184`, PR #22 `0316dc4`/`7b97706`, PR #23 `55ac6b0`/`798c25f`)이 이미 이 작업 전부(README 라이브검증 표기 정정 + 변경이력 재정렬 + 라이선스 안내 추가)를 완료해 `main`에 병합·푸시까지 끝낸 상태였음. README.md를 직접 grep한 결과 해당 항목은 이제 631·632줄로 옮겨져 `✅ 완료(2026-09-01 라이브 검증)`로 이미 정정돼 있고, §13·§17 안내문도 전부 갱신 완료(추측 아님, 직접 대조). **이 초안 자체가 이미 끝난 일을 "할 일"로 잘못 기록한 낡은 상태였음** — 이 프로젝트가 2026-08-18부터 반복 지적해 온 "문서가 실제보다 뒤처짐" 패턴이 이번엔 그 패턴을 추적하는 이 파일 자신에게 재발한 것.
- **신규 발견(이번 세션에서 처음 확인, 과거 기록엔 없던 것)**: `git ls-remote --tags origin`·`gh release list`를 직접 실행해 확인한 결과, **`v0.10.0` 태그는 원격에 push까지 완료**됐지만 **GitHub Release는 여전히 `v0.9.0`이 "Latest"** — v0.10.0 Release가 미발행 상태. 아래 "범위 밖"에 있던 기존 서술("필요 여부만 보고 완료된 상태, 급하지 않음")은 사용자의 기존 범용 규칙("모든 프로젝트 릴리즈 항상 최신 유지")과 실제로 어긋나 있어 우선순위를 재조정함(아래 1순위 참고). `gh release create` 자체는 이 파일의 기존 정책대로 AI가 실행하지 않음 — 명령/근거만 준비해 사용자에게 전달.
- **조치**: 아래 "다음 단계"를 2026-09-11 기준으로 갱신, 이 파일 자체를 커밋.

## 🟢 2026-09-11 갱신(2) — PRD-vs-실제코드 전수 감사 + 볼트 재스캔 캐싱 구현(성능 개선)

> 사용자 요청("지금까지 개발한 기준으로 PRD 문서대로 전부 구현됐는지 확인, 장점은 극대화·단점은 개선")으로 문서끼리 대조가 아니라 **PRD 약속 vs 실제 코드**(`mcp/lib/*.mjs` 9개 1,868줄 + 스킬 6개)를 파일:줄 단위로 전수 대조.

- **감사 결론**: 옵시디언 쪽(수집·분류·요약·링크·MOC·안전게이트·데이터모델 12개 frontmatter 필드)은 PRD 그대로 코드로 완전 구현·테스트됨(215/215). 노션 쪽(Research Library·Run Log)은 **설계상 의도적으로 코드가 아니라 스킬 프롬프트에만** 구현돼 있어("코어 밖" — `.PRD/README.md:14`) 자동 회귀 테스트가 원천적으로 없음 — 버그가 아니라 PRD 자신의 설계 결정. 새 코드 버그는 발견되지 않음.
- **발견된 개선 여지(코드 품질)**: `classify.mjs`(`loadAll`)·`link.mjs`(`loadNotes`)·`lint.mjs`·`collect.mjs`(`findDuplicate`) 4곳이 볼트 전체를 훑을 때마다 노트를 전부 다시 읽고 파싱 — 캐싱 0. PRD `01_PRD.md:14-17`이 풀려는 문제("손 정리는 시간이 많이 들어 결국 안 하게 된다")가 볼트가 커지면 도구 자신의 속도로 재현될 위험.
- **조치**: `mcp/lib/shared.mjs`에 `readFileCached()` 추가 — 절대경로 키 `Map`에 `{mtimeMs, size, text}` 캐시. 파일 발견(`walkVault`)은 전혀 안 건드리고(추가·삭제 파일 여전히 매번 정확히 탐지), "내용을 다시 읽을지"만 mtime+size로 판단. 무효화 로직을 따로 안 둔 이유: 우리 도구의 쓰기(`writeFileAtomic`)도 외부 편집(옵시디언)도 전부 mtime을 갱신하므로 다음 호출에서 자동으로 캐시 미스 — "무효화를 깜빡해서 낡은 값 반환" 사고 유형이 구조적으로 불가능. 4개 호출부 교체(커밋 `ba0d0d4`).
- **실수·수정**: 최초 편집 시 `link.mjs`에 `readFileCached` import 추가를 빠뜨려 `npm run verify` 중 `ReferenceError`로 즉시 발각(exit 1) — import 추가 후 재실행해 통과 확인. (완료 보고 전 실제 재검증을 돌리지 않았다면 놓쳤을 결함이라 기록.)
- **검증**: `npm run verify` 215/215 · `smoke-server.mjs`+`smoke-tools.mjs`(18/18, 한 서버 프로세스 안에서 쓰기→재읽기 반복 시나리오 다수 포함) · 실볼트 e2e 5종(link/moc/classify/summarize/collect-cli) · `security-scan.mjs --all` 70/70 — 전부 PASS, 회귀 0. 격리된 마이크로벤치마크(파일 400개)로 캐시 히트 시 88.4% 단축(218.4ms→25.4ms) 실측 확인.
- **범위**: 성능만, 새 기능·데이터모델·안전게이트 변경 0(04_PROJECT_SPEC.md "절대 하지 마" 목록 무영향).
- push 완료(`main == origin/main`, `ba0d0d4`).

## 🟢 2026-09-11 갱신(3) — 스킬-Run Log 정합성 자동검사 신설 + 실제 배선 누락 1건 발견·수정, 문서 수치 동기화

> 사용자 승인("캐싱 다음의 새 개선 방향을 또 찾아서 진행")으로 위 갱신(2)의 D절 개선 후보 중 "6개 SKILL.md의 노션 Run Log 필드 스키마 일관성 검사"를 착수.

- **조치**: `scripts/check-skill-triggers.mjs` 신설(읽기전용) — `classify`/`link`/`summarize` 3개 스킬의 "## 도구" 절에 정의된 각 쓰기 action이 그 스킬 자신의 "### 노션 Run Log" 범위 서술에 실제로 언급돼 있는지 자동 대조. `npm run verify` 체인 끝에 편입(`package.json`)해 CI에도 자동 포함됨.
- **즉시 발견(실측, 추측 아님)**: `wikimate-link/SKILL.md`의 `set_notion_id` action(2026-08-17 추가)이 그 파일 자신의 Run Log "범위" 문장(`add_links`/`build_moc`만 언급)에서 빠져 있었음 — 이 프로젝트가 2026-08-18(MOC 트리거)·2026-08-31(status/project 트리거)·2026-08-21(2)(Run Log 5개 스킬 확장)에서 세 번 반복 발견했던 것과 동일한 결함 유형("도구엔 있는데 트리거/로그 서술엔 없음")의 네 번째 사례. 실무 영향은 제한적(`set_notion_id`는 보통 Organize 워크플로우 경유로 호출돼 그쪽 Run Log 범위가 이미 커버하지만, 이 스킬을 통한 단독 호출 시엔 안전 불변 조건 #6 "Run Log 매 변경 기록"을 놓칠 수 있었음).
- **수정**: `set_notion_id`를 범위 문장에 추가 + 발견 경위 한 줄 명시. `npm run verify` 총계 **215→223**(신규 체크 8개: 스킬 3개 × Run Log 절 존재 확인 1개 + action별 확인, 전부 PASS).
- **문서 동기화**: `README.md`/`README.en.md`(md+html 4파일)의 "현재 상태" 배너·`npm run verify` 안내 주석 2곳씩(총 3곳×4파일=12곳)을 215→223으로 갱신. **과거 changelog 토글 항목(§13, "206→215" 등 날짜 붙은 기록)은 그 시점 사실이라 건드리지 않음**(이 파일 자신의 "과거 기록 임의수정 금지" 원칙과 동일). HTML 2개는 이번엔 pandoc 전체 재생성 대신 **동일 3곳만 직접 치환**했음(헤딩·섹션 구조 변화가 전혀 없는 순수 숫자 교체라 재생성 없이도 md↔html 완전 일치 — h2(22)·h3(33)·details(11) 개수 불변 확인, "223" 등장 횟수 md/html 각 3회로 정확히 일치 확인. 구조가 바뀌는 변경이었다면 이 방식 대신 기존 pandoc 재생성 방법을 썼을 것).
- **검증**: `npm run verify` 223/223 · `security-scan.mjs --all` 71개(신규 스크립트 1개 추가로 70→71) 통과. 코드(`mcp/lib/*.mjs`) 변경 0 — 스킬 프롬프트·테스트 도구·문서만.
- push 완료(`main == origin/main`).

## 다음 단계 (2026-09-11 확정(2) — 최신, 아래가 최신 우선순위. 위 2026-09-11(1)·2026-09-01 절은 참고용 이력)

### 1순위 · `v0.10.0` GitHub Release 발행 (태그는 이미 있음, 노트만 필요 — 사람 전담, 유일하게 남은 항목)
**왜 1순위인가**: 코드·문서 변경이 전혀 필요 없고(태그·`package.json` 이미 `0.10.0`), 사용자의 기존 범용 규칙과 직접 어긋난 상태로 방치 중인 유일한 항목. 위험 대비 효과가 가장 명확함.
**AI가 준비한 것**: 0.9.0 이후 변경분 요약(status/project 자연어 트리거, `Link.reason`/`Link.kind` 구현, 노션 Run Log 5개 스킬 확장, MOC 중복·`replace_link` 존재검증 등 실결함 수정, 노션·마켓플레이스 라이브 검증 완료, 볼트 재스캔 캐싱) — 실행은 사용자가 `gh release create v0.10.0` 직접 실행(정책상 AI 실행 금지).
**위험**: 낮음(공개 저장소에 보이는 행동이라 최종 문구는 사용자 확인 필요).

### ~~2순위 · 이 `CHECKPOINT.md` 커밋~~ ✅ 완료(2026-09-11, 커밋 `8b7480b`)

### 범위 밖 (명확히 구분 — 다음 세션에서 먼저 꺼내지 말 것)
- Codex/Gemini 라이브 포팅: 사용자 보류 결정, 준비됐다고 먼저 말할 때까지 대기.
- Python 고급 추출기: 계획만 있고 미착수, 우선순위 낮음(사용자가 요청 안 하면 제안하지 않음).
- 노션 색인·Run Log 코드화: PRD가 의도적으로 "코어 밖"으로 설계(스킬 프롬프트 전담) — 코드로 전환 제안하지 않음(범위 확대 위험).
- `.active-agents*`(3개)·`.sodam-re/safety-log.jsonl` 미추적 파일: `.gitignore` 미등록 상태로 여러 세션째 방치 확인(2026-09-11). `.gitignore` 등록 여부·삭제 여부 결정 필요하나 이번 지시 범위 밖이라 판단만 남기고 손대지 않음.
