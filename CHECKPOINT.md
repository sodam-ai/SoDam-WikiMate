...
- **재검증**: `scripts/verify-link.mjs`에 회귀 9개 추가(정확한 bullet 줄 수 검증 포함, 캐논 헤딩·레거시 alias 헤딩 둘 다) — 50→**59**. `npm run verify` 총계 **180→189**(전부 통과). 실볼트 `e2e-moc-real.mjs` 재실행 결과 중복 카운트가 더 안 늘어남을 직접 확인(11/10에서 고정). `sandbox-vault/`의 기존 중복 잔재는 손대지 않음(gitignore 대상 로컬 픽스처, 데이터 손실 아님 — 앞으로 더 안 늘어나는 것만 확인).
...
- **재검증**: `scripts/verify-fix.mjs`의 기존 두 테스트(`연결대상`/`새이름`)가 사실은 존재하지 않는 가짜 이름으로 성공을 검증하고 있었던 것도 이번에 발견 — 두 이름을 실제 fixture 노트로 추가해 원래 테스트 의도(치환 메커니즘 자체 검증)는 그대로 보존하면서 새 안전장치와 호환되게 정정. 신규 회귀 2개 추가(존재하지 않는 `to` 거부 + 거부 후 파일 미변경). `npm run verify` 총계에 이미 반영(위 189에 포함, verify-fix 16→18).

### 공통 재검증
`npm run verify` **189/189**(exit 0) · `node --check` 전체 clean · `smoke-server`/`smoke-tools`(14/14) · `security-scan.mjs --all` 72개 통과 · `obsidian.json` 해시 세션 시작 시점과 100% 동일. 두 결함 모두 **이번 세션에서 새로 만든 코드가 아니라 2026-07-11(M2)·2026-06-22(fix.mjs 최초 구현) 시점부터 있던 것**을 재검증 과정에서 실측으로 발견한 것 — 이번 라운드에서 만든 Run Log 스킬 문서 확장(갱신(2))과는 무관.
