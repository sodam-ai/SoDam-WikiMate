...
AI 에이전트에게 **"정리해줘"** 라고 하면, 흩어진 자료(웹 링크·PDF·대화 로그·텍스트)를 **내 옵시디언 볼트에 노트로 정리**하고, 관련 있는 노트끼리 **연결**하고, **분류**까지 해 주고, 원하면 **노션에 색인**해주는 **Claude Code 플러그인**. (Codex엔 "플러그인 설치(`/plugin`)"가 없어요 — 대신 저장소를 받아 `codex mcp add`로 **MCP 서버로 등록**해 연동합니다: [`adapters/codex/SETUP.md`](./adapters/codex/SETUP.md).)

**[English](./README.en.md)**

> 📘 **처음이세요?** 그대로 따라 하는 [왕초보 가이드](./GUIDE.ko.md) ([HTML](./GUIDE.ko.html))를 보세요 — 용어 사전·설치·사용·문제해결·FAQ·라이선스까지 전부 쉽게 설명해요.

> 상태: **v0.7.2 기반 + 다음 버전 예정 신규 기능 3종**(자동 링크·MOC·자동 분류, MCP 도구 2개 추가로 총 7개 — 이 저장소에 구현·자동 테스트 119개 통과 완료, 옵시디언 그래프뷰 실제 표시·세션 재시작 라이브 확인은 사용자 확인 대기 중). 정리·검색·볼트 건강검진·작업 로그·보관함 자동탐지는 동작·검증 완료. 아래 "업데이트 내용" 참고. 노션 색인은 노션 도구가 연결된 환경에서 동작(아래 "현재 상태" 참고).

> 📱 **기기 안내:** Wikimate는 **윈도우 PC(데스크톱·노트북) 전용**이에요. **스마트폰·태블릿에는 설치되지 않습니다.**

## 무엇을 해주나요?
- 🧹 **자연어로 정리** — "이 링크 정리해줘" 한마디면 노트가 만들어져요. (가끔 자동발동이 안 되면 **`/wikimate <링크>`** 슬래시 명령으로 100% 실행.)
- 🧭 **보관함 자동탐지** — 옵시디언에 등록된 보관함(볼트)을 자동으로 찾아 **"여기에 정리할까요?"** 하고 제안해줘요(이름 안 말해도 됨, 도구 `wikimate_vaults`).
...
- 🆕 🔗 **자동 링크·MOC** — 관련 있는 노트끼리 `[[링크]]`로 연결(노트당 최대 5개, 과잉 연결 방지)하고, 같은 주제 노트는 목차 노트(MOC)로 묶어줘요.
- 🆕 🗂️ **자동 분류** — 미분류 노트의 폴더·태그·중요도를 판단해 제안하고, 승인하면 적용해요.
- 🆕 🔔 **세션 시작 안내** — Claude Code를 켤 때 옵시디언 도구·등록된 보관함을 자동 감지해 안내(읽기 전용).
- 🆕 🕵️ **검수 서브에이전트** — 실제로 노트를 쓴 뒤 "완료" 보고 전에, 별도 관점의 AI가 원문 왜곡·인젝션 감염·기존 노트 훼손 여부를 한 번 더 확인.

<details>
<summary><b>📋 업데이트 내용 요약 (펼쳐서 보기)</b></summary>

**✅ v0.7.1 (정식 배포판)**
- `wikimate_collect`(정리) · `wikimate_lint`(건강검진) · `wikimate_fix`(안전수정) · `wikimate_runlog`(작업기록) · `wikimate_vaults`(보관함탐지) — MCP 도구 5개
- 노션 선택 색인, 스킬 3종(organize/query/lint)

**🆕 다음 버전 예정 (이 저장소에 구현·검증 완료, 정식 배포 전)**
- `wikimate_link`(자동 링크·MOC) · `wikimate_classify`(자동 분류) — MCP 도구 2개 추가(총 7개)
- SessionStart hook(세션 시작 자동 감지) · `wikimate-reviewer` 검수 서브에이전트
- 자동 테스트 119개 전부 통과(단위 테스트 + 실제 서버 프로토콜 + 실제 볼트 픽스처 E2E), 실행 중 발견된 결함 5건(셀프링크·중복입력·중요도값 미검증 등) 전부 수정·회귀테스트로 고정됨
- ⚠️ 남은 미확인 항목: 실제 옵시디언 그래프뷰/백링크 화면 표시, Claude Code 재시작 후 SessionStart 알림 — 둘 다 사용자 환경에서 직접 확인 필요(자동화로 확인 불가능한 영역)
...
```
설치 후 Claude Code를 재시작하면 끝. (확인하고 싶으면 `/mcp`에 `wikimate_collect`·`wikimate_lint`·`wikimate_fix`·`wikimate_runlog`·`wikimate_vaults` 5개가 보이면 성공. 🆕 신규기능 포함 폴더로 설치했다면 `wikimate_link`·`wikimate_classify`까지 7개.)

## 실행 방법
**따로 "실행(켜기)"할 게 없어요.** 설치만 하면 Claude Code를 켤 때마다 자동으로 함께 켜져요. "실행"은 곧 **Claude Code를 켜고 대화창에 말로 시키는 것**이 전부예요. (개발자가 서버를 직접 돌리려면 받은 폴더에서 `npm install` 후 `npm start`. 전체 자동 검증은 `npm run verify`.)
...
> ⭐ **확실한 방법(권장)** — 자연어 "정리해줘"는 AI 판단이라 **가끔 자동발동을 안 하고 그냥 요약만** 할 때가 있어요. 그럴 땐 **슬래시 명령** `/wikimate <링크/텍스트>` 를 쓰면 **100% 발동**합니다(직접 명령이라). 예: `/wikimate https://example.com`

- 📎 **정리할 "자료"는 무엇이든 돼요** — 한 문장 텍스트, 웹 링크(`이 링크 정리해줘: https://...`), 파일 경로(`이 파일 정리해줘: D:\메모\오늘.md`) 모두 OK. 따로 준비할 게 없어요.
- 🧭 **보관함 이름은 안 말해도 돼요** — Wikimate가 등록된 보관함을 찾아 "여기에 할까요?"라고 제안해줘요. 콕 집어 *"내 'Vault' 볼트에"* 처럼 말해도 됩니다(이름은 옵시디언 좌하단 볼트 전환 메뉴에서 확인).
...
**🆕 관련 노트 연결·목차 만들기도 대화로**:
> "이 노트랑 관련 있는 노트들 연결해줘" / "MCP 관련 노트들 목차로 묶어줘"

- 볼트 전체에서 후보를 찾아 **제목·요약·태그만** 보고(본문은 안 봄) 관련도를 판단·제안하고, 승인한 것만 연결(노트당 최대 5개). 목차 요청 시 주제별 목차 노트(MOC)를 생성·갱신(기존 사용자 작성 내용은 보존).

**🆕 자동 분류도 대화로**:
> "이 미분류 노트들 정리해줘"

- 폴더·태그·중요도를 제안하고, 승인하면 적용. 파일명 충돌 시 절대 덮어쓰지 않음.
...
mcp/server.mjs         무의존 MCP 서버 (stdio) — 도구 7개(collect·lint·fix·runlog·vaults·link·classify)
mcp/lib/shared.mjs     🆕 공통 헬퍼(경로안전검사·백업·frontmatter 파싱)
mcp/lib/collect.mjs    수집(이름 안전화·중복 차단)
mcp/lib/lint.mjs       건강검진(읽기전용)
mcp/lib/fix.mjs        안전 수정(삭제 없음·백업)
mcp/lib/runlog.mjs     작업 로그
mcp/lib/link.mjs       🆕 자동 링크·MOC
mcp/lib/classify.mjs   🆕 자동 분류
skills/                자연어 자동 발동 스킬(organize·query·lint·link·classify)
commands/              /wikimate · /wikimate-lint · /wikimate-link · /wikimate-classify 명령
hooks/                 🆕 세션 시작 자동 감지
agents/                🆕 검수 서브에이전트(wikimate-reviewer)
...
## 아키텍처 (간단히)
```
[나] --말로 명령--> [Claude Code/Codex] --MCP--> [Wikimate 서버] --> [collect/link/classify 등] --> [내 옵시디언 볼트]
```
관련도·분류 판단은 **AI(Claude)가 직접** 하고, 서버는 후보 정보 제공과 안전한 파일 쓰기만 담당합니다(별도 유사도 엔진 없음 — 중복 개발 방지). 자세한 그림은 [왕초보 가이드 14번](./GUIDE.ko.md).
...
- 🧾 **원문 보존 알림** — 정리할 자료에 웹주소는 있는데 내용이 너무 짧으면("요약만 넣은 거 아니에요?") 또는 너무 크면, 저장 전 계획(dry-run) 화면에 알려드려요. **차단이 아니라 알림**이에요 — 확인 후 그대로 진행해도 되고, 원문을 다시 준비해 다시 정리해도 돼요.
...
- ✅ 🆕 노트당 관련 링크는 **최대 5개**로 코드가 직접 강제(과잉 연결 방지, 프롬프트 지시만으로는 불충분해 코드 레벨 검증).
- ✅ 🆕 숫자 입력값(중요도 등)은 범위·타입 검증 후 안전한 기본값으로 대체(2026-07-11 실측 검증으로 발견·수정).
...
자세한 보안·데이터 흐름은 [왕초보 가이드 15번](./GUIDE.ko.md).

## 현재 상태 (정직하게)
- ✅ **옵시디언 정리**: 실제 볼트에 노트 생성 확인(자연어 자동 발동 포함).
- ✅ **건강검진·안전수정·작업로그**: 단위테스트 + 서버 프로토콜 e2e로 검증(중복·깨진링크·고아 탐지, 삭제 없는 수정, 자동 기록).
- ✅ 🆕 **자동 링크·MOC·자동 분류**: 자동 테스트 119개 통과, 실제 볼트 픽스처로 E2E 검증(인젝션 방어·5개 상한·기존 콘텐츠 보존 등 실측 확인). 실행 중 실제 결함 5건 발견 후 전부 수정·회귀테스트 고정.
- 🟡 🆕 **그래프뷰 반영·hook 라이브 로드**: 코드·자동테스트로는 확인 한계 — 사용자가 실제 옵시디언 앱·Claude Code 재시작으로 직접 확인 필요.
- 🟡 **노션 색인**: 노션 도구(MCP/CLI)가 연결·인증된 환경에서 동작. 환경에 따라 미연결이면 자동으로 건너뜁니다.
- 🟡 **Codex**: 같은 MCP 코어를 쓰도록 어댑터 제공(설정은 `adapters/codex/SETUP.md`). 스킬(자동발동)은 Claude Code 전용.
- ⚠️ **라이브 세션 미검증**: 실제 Claude Code `/mcp` 세션·실볼트 검증은 사용자 환경에서 권장(2분, 위 설치 참고).

## 문제 해결
- **"정리해줘" 했는데 노트는 안 만들고 요약만 해요** → 자연어 자동 발동이 들쭉날쭉이에요. **`/wikimate <링크>`** 슬래시 명령으로 콕 찍으면 100% 됩니다(가장 확실).
...
- **🆕 연결이 5개 넘게 안 돼요** → 의도된 안전장치예요(과잉 연결 방지).

> 더 많은 증상·해결·FAQ는 [왕초보 가이드 16·17번](./GUIDE.ko.md)에 표로 정리돼 있어요.
...
- **Wikimate가 정리해 준 내 노트의 저작권은 100% 사용자 것**이에요. Wikimate(SoDam AI Studio)는 정리 결과물(노트)에 대해 어떤 권리도 주장하지 않아요 — Wikimate는 소프트웨어일 뿐, 그 결과로 만들어진 노트의 저작자가 아닙니다.

> 전체 라이선스 표·면책은 [왕초보 가이드 19번](./GUIDE.ko.md) 또는 `NOTICE` 참고. 개발·테스트·배포 방법은 [DEVELOPMENT.md](./DEVELOPMENT.md).
