...
1. **수집 대상 파악**: 웹 링크면 내용을 읽어(WebFetch) 제목·요약·본문 준비. 파일이면 텍스트 추출.
   ...
   - **자료가 여러 개(2개 이상)면 소스별로 병렬 서브에이전트를 띄운다**(각 서브에이전트가 자료 하나씩 `wikimate_collect` 담당). 한 자료가 실패해도 다른 자료 수집에 전파되지 않게 격리한다(04_PROJECT_SPEC.md §3.5). 자료가 1개뿐이면 병렬화하지 않는다(과잉설계 금지).
