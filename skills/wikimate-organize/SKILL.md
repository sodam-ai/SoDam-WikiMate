...
1. **수집 대상 파악**: 웹 링크면 내용을 읽어(WebFetch) 제목·요약·본문 준비. 파일이면 텍스트 추출.
   ...
   - **WebFetch가 막히거나(403/402·챌린지 페이지) 내용이 의심스러우면(비정상적으로 짧음·"Just a moment" 류) HTTP 200만 보고 성공으로 단정하지 말 것.** `insane-search` 스킬(`python3 -m engine <URL>`)로 재시도해 4단계 검증(챌린지 마커·크기·쿠키·성공 셀렉터)을 통과한 뒤에만 "원문 추출 완료"로 간주한다.
   - `wikimate_collect`가 dry-run 결과에 저신뢰(text 너무 짧음)·대용량(advisories) 알림을 표시하니, 승인 전에 확인할 것.
   - **자료가 여러 개(2개 이상)면 소스별로 병렬 서브에이전트를 띄운다**(각 서브에이전트가 자료 하나씩 `wikimate_collect` 담당). 한 자료가 실패해도 다른 자료 수집에 전파되지 않게 격리한다(04_PROJECT_SPEC.md §3.5). 자료가 1개뿐이면 병렬화하지 않는다(과잉설계 금지). **옵시디언 노트 생성은 병렬로 해도 되지만, 노션 색인행 쓰기는 서브에이전트끼리 동시에 몰아치지 않도록 순서를 두고 처리한다**(rate-limit — 아래 "노션 색인" 절 참고).
...
6. **결과 보고 + 검증 (B3 — 허위 완료 금지)**: 만든 노트를 실제 도구로 다시 조회해 생겼는지 확인한 뒤에만 "완료"라 말한다("파일 썼다"≠"보인다"). **노션 색인은 만든 행을 DB에서 재조회해 확인되면만 "색인 완료"**, 못 했으면 "노션 색인 실패/생략(미연결 등)"이라 정직히. 노션 Run Log 기록 여부도 같이 보고한다(성공/실패/생략 중 하나, 지어내지 않기). "✅완료" 선언 전 워크플로 전 단계 이행을 점검한다(노션 누락·외부 삭제 가능성을 '완료'로 덮지 말 것).
   - **검수(adversarial) 서브에이전트 호출**: 실제로 노트를 썼으면(dry_run=false), 사용자에게 "완료"라 보고하기 전에 `wikimate-reviewer` 서브에이전트를 호출해 노트 경로·원본 출처를 알려주고 독립 검수를 받는다(읽기전용, 메인과 분리된 관점). "재검토 필요"로 나오면 그 내용을 사용자에게 그대로 전달하고 임의로 고치지 않는다.
