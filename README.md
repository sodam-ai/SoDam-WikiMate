...
> 상태: **v0.7.1(정식 배포)** — 정리·검색·볼트 건강검진·작업 로그·보관함 자동탐지(MCP 도구 5개) 동작·검증 완료. **+ 다음 버전 예정 신규 기능 3종**(자동 링크·MOC·자동 분류, MCP 도구 2개 추가로 총 7개 — 이 저장소에 구현·자동 테스트 119개 통과 완료, 옵시디언 그래프뷰 실제 표시·세션 재시작 라이브 확인은 사용자 확인 대기 중). 아래 "업데이트 내용" 참고. 노션 색인은 노션 도구가 연결된 환경에서 동작(아래 "현재 상태" 참고).
