...
- [x] **`Link.reason` 저장 방식**(2026-08-31 확정) — frontmatter `related:`가 아니라 **노트 본문의 "## 왜 연결했는지" 섹션에 병행 기록**한다(`- [[대상노트]] — 이유`). `related:` 파서가 "한 줄 배열만" 읽는 자체 정규식이라, frontmatter 쪽에 구조를 넣으려면 기존에 저장된 모든 노트의 값과 호환이 깨지는 마이그레이션이 필요했음(위험 대비 이득 작음) — 대신 본문 병행은 무위험·되돌리기 쉬움. `reason`을 안 넘기면 섹션 자체가 생기지 않아 기존 노트에 영향 없음(하위호환). `Link.kind`(related/reference 종류 구분)는 여전히 미구현·미결(범위 밖으로 별도 판단 필요).
